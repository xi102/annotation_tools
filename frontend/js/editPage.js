...
angular.module("app", []).controller("table", function ($scope, $http) {
    var urls = "http://139.224.54.233:5000/api/v1/";
    //本地导入 已废弃
    ...
    $scope.entities_confirmed =  [];
    $scope.showNums = false;
    $scope.categories = ["疾病",  "症状","异常检查结果","检查","治疗","身体部位","时间"];
    ...
    $scope.reviewEntity = function () {
        $http({
            ...
            data: {
                content_id: document.getElementById("fileId").value
                /**/
            }
        }).then(function successCallback(response) {
            if (response.data.flag == 1) {
                ...
                // }
                $scope.entities_confirmed = response.data.entities_confirmed;
    ...
    $scope.pushChecked = function () {
        ...
        for (var j = 0; j < $scope.entityList.length; j++) {
            if ($scope.entityList[j].category) {
                jsons.entities.push({
                    ...
                    "decoration":$scope.entityList[j].decoration
                })
        ...
        for(var k=0;k<$scope.entities_confirmed.length;k++){
            jsons.entities.push({
                "name":$scope.entities_confirmed[k].name,
                "startPosition": $scope.entities_confirmed[k].startPosition.toString(),
                "endPosition": $scope.entities_confirmed[k].endPosition.toString(),
                "category": $scope.entities_confirmed[k].category,
                "decoration":$scope.entities_confirmed[k].decoration
            })
        }
        if ($scope.entityList.length > 0) {
            ...
            $http({
                method: "post",
                url: "http://139.224.54.233:5000/api/v1/refreshEntityList",
    ...
    $scope.change = function () {
        ...
        if(selectedValue =="疾病"||selectedValue=="症状"||selectedValue=="异常检查结果"){
            $scope.level_2_options[index] = ["肯定的","否认的","可能的"];
    ...
    $scope.pushToDB = function () {
        ...
        if ($scope.entityList.length > 0) {
            ...
            $http({
                method: "post",
                url: "http://139.224.54.233:5000/api/v1/",
                data: JSON.stringify(jsons)
            }).then(function successCallback(response) {
                if (response.data.flag == 1) {
                    alert("上传完成！")
